...
import { fetchStravaTyreProfile, type RecoView, type TyreView } from "@/lib/api";
import { QUESTIONS, answersToApiParams, getOptions, inferredToAnswers, type Answers } from "@/lib/questions";
import { recommendTyresFromCatalog } from "@/lib/recommendations";
import { cn } from "@/lib/utils";
import { supabase } from "@/lib/supabase/client";

type Phase = "quiz" | "loading" | "results";
const MAX_COMPARE_SELECTED = 3;

export function TrouveTonPneuClient({ catalog }: { catalog: TyreView[] }) {
  ...
  async function handleNext() {
    ...
    setError(null);
    try {
      const items = recommendTyresFromCatalog(catalog, answersToApiParams(answers));
      setResults(items);
      if (items.length > 0) {
  ...
  function toggleSelect(id: string) {
    setSelectedIds((prev) => {
      ...
      else if (next.size < MAX_COMPARE_SELECTED) next.add(id);
      return next;
    });
  }
...
function ResultsPhaseView({
  ...
  const compareHref = `/comparateur?ids=${encodeURIComponent([...selectedIds].slice(0, MAX_COMPARE_SELECTED).join(","))}`;
